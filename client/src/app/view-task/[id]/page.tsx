...
const TaskPage = ({ params }: Props) => {
  const { data: task, isLoading: getTasksLoad } = useGetTaskQuery({ id: params.id });
  ...
  return (
    <div className="max-w-xl mx-auto my-10 p-6 bg-white rounded-lg shadow-md mt-32 font-Poppins">
      <h1 className="text-4xl font-bold mb-4 text-center text-gray-800">{task?.title}</h1>
      <div className="mb-4 text-gray-700">
        <p className="text-lg">{task?.description}</p>
      </div>
      <div className="mb-4">
        <span className="font-bold text-gray-700">Status:</span>
        <span className={`ml-2 px-2 py-1 rounded-full ${getStatusColor(task?.status)}`}>
          {task?.status}
        </span>
      </div>
      <div className="mb-4">
